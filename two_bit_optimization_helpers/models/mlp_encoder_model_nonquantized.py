...
def Mlp_Slim(shape):
    ...
    stack = _var_network(stack, hidden=16, output=3) # this network should only be used with 'slim' (3) or 'full' (8) regression targets
...
def Mlp_Full(shape):
    x_base = x_in = Input(shape, name="input_pxls")
    stack = _mlp_encoder_network(x_base)
    stack = _var_network(stack, hidden=16, output=8) # this network should only be used with 'slim' (3) or 'full' (8) regression targets
    model = Model(inputs=x_in, outputs=stack, name="smrtpxl_regression")
    return model

def Mlp_Slim_SoftQuantizer(shape, initial_thresholds, threshold_offset, initial_levels=None):
    x_base = x_in = Input(shape, name="input_pxls")
...
def Mlp_Full_SoftQuantizer(shape, initial_thresholds, threshold_offset, initial_levels=None):
    x_base = x_in = Input(shape, name="input_pxls")
    x_base = SoftQuantizeLayer(
        n_bits=2,                     
        initial_thresholds=initial_thresholds,
